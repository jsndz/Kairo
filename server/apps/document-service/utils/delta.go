...
func CombineDeltaState(currentState []byte, deltas *[]model.DocumentUpdate) ([]byte, error) {
	...
	updates := make([]string, len(*deltas))
	...
	resp, err := client.Post("http://localhost:3005", "application/json", bytes.NewReader(data))
	if err != nil {
		return nil, err
	}
	...
	return base64.StdEncoding.DecodeString(res.Merged)
